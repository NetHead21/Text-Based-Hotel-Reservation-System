...
def get_file_index(file_name: str) -> int:
    try:
        with open(file_name, "r") as file_writer:
            for count, _ in enumerate(file_writer):
                pass
            return count + 2
    except Exception:
        return 1
...
def delete_data(file_name: str, guest_name: str) -> bool:
